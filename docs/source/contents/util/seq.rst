...
* パターンをモードに従って展開
* ホールドの回数によるフィルタリング (``--not-expression`` オプション)
* シーケンスサイズの変換処理 (``--length`` オプション)
* ミノの個数によるフィルタリング (``--not-expression`` オプション)
* 正規表現によるフィルタリング (``--expression`` オプション)
* シーケンスの出力
...
なお ``--not-expression`` を同時に指定した場合、2つのオプションを満たすもの（AND）が取り出されます。
...
なお ``--expression`` を同時に指定した場合、2つのオプションを満たすもの（AND）が取り出されます。
...
* ``--head-hold yes`` を指定すると、``L`` がホールドされている状態で ``Z`` を受け取るところから開始される
* ``--head-hold no`` を指定すると、何もホールドされていない状態で ``L`` を受け取るところから開始される
